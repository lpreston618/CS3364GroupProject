...
def insertionSort(array: list[int]):
    for i in range(len(array)):
        ...
        for j in range(i, 0, -1):
            # Swap the i_th element down until it finds its place in
            # the sorted subarray
            if array[j] < array[j - 1]:
                temp = array[j]
                array[j] = array[j - 1]
                array[j - 1] = temp
            # Once it finds its place, stop the inner loop and move
            # on to the next element.
            else:
                break


# Partition a list according to the quicksort algorithm. Uses the
# middle element as a pivot. Returns three partitions: the elements
# lesser than the pivot, elements equal to the pivot, and elements
# greater than the pivot. Assumes that array has at least two
# elements - it's up to the algorithms calling partition to ensure
# correct usage.
...
# Our hybrid sort - quicksorts until the array length is less than
# or equal to k, our threshold, then we hand it over to the
...
def quicksortHybrid(array: list[int], threshold: int) -> list[int]:
    if len(array) <= threshold:
        ...
        return array
    left, same, right = partition(array)
    return quicksortHybrid(left, threshold) \
        + same + quicksortHybrid(right, threshold)
